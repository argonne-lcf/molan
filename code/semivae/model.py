...
def make_mlp(start_dim, end_dim, n_layers, ratio, act, batchnorm, dropout):
    layer_sizes = [int(np.ceil(start_dim * ratio**i))
                   for i in range(n_layers + 1)] + [end_dim]
    layers = []
    for index in range(n_layers):
        layers.append(nn.Linear(layer_sizes[index], layer_sizes[index + 1]))
        layers.append(str2activation(act))
        if batchnorm:
    ...
    layers.append(
        nn.Linear(layer_sizes[n_layers], layer_sizes[n_layers + 1]))
...
class SemiVAE(nn.Module):
    ...
    def __init__(self, vocab, config):
        ...
        self.decoder_y = make_mlp(config.d_z, 1,
                                  config.y_n_layers,
                                  config.y_layer_ratio_size,
                                  config.y_act,
                                  config.y_batchnorm,
                                  config.y_dropout)
    ...
    def tensor2string(self, ids):
        string = self.vocabulary.ids2string(
            ids.tolist(), rem_bos=True, rem_eos=True)
        return string
    ...
    def forward(self, x, y, y_mask):
        ...
        recon_loss, x_hat = self.forward_decoder(x, z)

        # Latent space to property
        y_loss, y_hat = self.forward_y(y, y_mask, z)
        return kl_loss, recon_loss, y_loss, x_hat, y_hat

    def z_to_y(self, z):
        return self.decoder_y(z)

    def forward_y(self, y, y_mask, z):
        ...
        y_hat = self.z_to_y(z)
        y_loss = masked_mse_loss(y_hat, y, y_mask)
